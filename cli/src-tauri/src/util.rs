...
pub fn get_package_name_from_config(config: &Config) -> String {
    config
        .product_name
        .clone()
...
pub fn get_runtime_config_path(app: &AppHandle, package_name: String) -> std::io::Result<PathBuf> {
    Ok(get_data_dir(app, package_name)?.join(RUNTIME_CONFIG_FILE))
}

pub fn read_runtime_url(path: &Path) -> std::io::Result<Option<String>> {
    if !path.exists() {
        return Ok(None);
    }

    let content = std::fs::read_to_string(path)?;
    let config: RuntimeConfig = serde_json::from_str(&content).map_err(|err| {
        std::io::Error::new(
            std::io::ErrorKind::InvalidData,
            format!("Failed to parse runtime config: {err}"),
        )
    })?;
...
pub fn write_runtime_url(path: &Path, url: &str) -> std::io::Result<String> {
    ...
    let parent = path.parent().ok_or_else(|| {
        std::io::Error::new(
            std::io::ErrorKind::InvalidInput,
            format!("Runtime config path has no parent: {}", path.display()),
        )
    })?;
    std::fs::create_dir_all(parent)?;

    let content = serde_json::to_string_pretty(&RuntimeConfig {
        current_url: Some(normalized_url.to_string()),
    })
    .map_err(|err| {
        std::io::Error::new(
            std::io::ErrorKind::InvalidData,
...
pub fn clear_runtime_url(path: &Path) -> std::io::Result<()> {
    if path.exists() {
...
mod tests {
    ...
    #[test]
    fn runtime_url_round_trip() {
        let path = temp_runtime_path();
        let saved = write_runtime_url(&path, " https://example.com/a?b=1 ").unwrap();
        assert_eq!(saved, "https://example.com/a?b=1");
        assert_eq!(
    ...
    #[test]
    fn runtime_url_clear_removes_saved_value() {
        let path = temp_runtime_path();
        write_runtime_url(&path, "https://example.com").unwrap();
        clear_runtime_url(&path).unwrap();
        assert_eq!(read_runtime_url(&path).unwrap(), None);
        let _ = fs::remove_dir_all(path.parent().unwrap());
    }

    #[test]
    fn runtime_url_empty_input_is_rejected() {
        let path = temp_runtime_path();
        let error = write_runtime_url(&path, "   ").unwrap_err();
