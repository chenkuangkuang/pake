...
use crate::util::{
    get_data_dir, get_package_name_from_config, get_runtime_config_path, read_runtime_url,
};
use std::{
    path::Path,
    path::PathBuf,
...
fn build_window_with_label(
    ...
    let package_name = get_package_name_from_config(tauri_config);
    let data_dir = get_data_dir(app, package_name.clone()).map_err(tauri::Error::Io)?;
    let runtime_config_path =
        get_runtime_config_path(app, package_name).map_err(tauri::Error::Io)?;
    let url = resolve_runtime_url(window_config, &data_dir, &runtime_config_path)?;
...
fn resolve_runtime_url(
    window_config: &crate::app::config::WindowConfig,
    _data_dir: &Path,
    runtime_config_path: &Path,
) -> tauri::Result<WebviewUrl> {
    match window_config.url_type.as_str() {
        "web" => {
            let runtime_url = read_runtime_url(runtime_config_path).map_err(tauri::Error::Io)?;
            let effective_url = runtime_url.unwrap_or_else(|| window_config.url.clone());
            let parsed = effective_url.parse().map_err(|err| {
                tauri::Error::Io(std::io::Error::new(
                    std::io::ErrorKind::InvalidInput,
                    format!("Invalid 'web' url '{}' in pake.json/runtime config: {err}", effective_url),
                ))
            })?;
